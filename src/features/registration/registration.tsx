...
export function Registration(props: { registration: TimeRegistration }) {
  ...
  const [start, setStart] = useState(timeFormatter.formatTimeFromDate(new Date(props.registration.start)));
  const [end, setEnd] = useState(timeFormatter.formatTimeFromDate(new Date(props.registration.end!)));
...
//  const end = new Date(props.registration.end!).toLocaleTimeString();
  ...
  function cancelRegistrationHandler() {
      setStart(timeFormatter.formatTimeFromDate(new Date(props.registration.start)));
  ...
  const body = editMode ?
      <Typography gutterBottom variant="body1" component="div">
          ...
          />
              <Button type="submit" variant="contained" className={styles.hidden}>
              </Button>
          </form>
      </Typography>
    :
